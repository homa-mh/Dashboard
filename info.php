...
function displayAnalyzedFields($data) {
    ...
    if (!empty($data['action_groups'][0]['action_group_name'])) {
        echo '<p style="display:inline-block; margin-right:10px">' . htmlspecialchars($data['action_groups'][0]['action_group_name']);
        if (!empty($data['unique_qr_code'])) {
        echo  " (کد: " . htmlspecialchars($data["unique_qr_code"]) . ")". '</p>';
        } else {
            echo '</p>';
        }
    } else {
